...
//                                   );
    ...
    public int accion(boolean PF, boolean PD, boolean PA, boolean PI, boolean MT) {
        ...
        if (pathcount >= 2) {
            ...
            Node actualNode = matrix.retriveNode(y,x);
            Data actualData;
            if (actualNode == null) {
                ...
                matrix.add(y, x, newData);
                actualNode = matrix.retriveNode(y,x);
            } else {
                actualData = (Data) actualNode.getData();
                if(x==0 || y == 0){
                    if (PA)actualData.changePaths((dir + 2) % 4, Data.CLOSED);
    ...
    public void updateCoor(int dir, int x, int y){
        if(dir==0){
            y--;
            return;
        }
